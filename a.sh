...
backend_docking_set(){
	...
	if [[ "${vnum}" == "1" ]]; then
        ...
        install_tool
        check_docker
        docker run -d --name=ssrmu -e NODE_ID=$node_id -e MU_SUFFIX=microsoft.com -e MU_REGEX=%5m%id.%suffix -e API_INTERFACE=modwebapi -e WEBAPI_URL=$web_url -e WEBAPI_TOKEN=$webapi_token --network=host --log-opt max-size=50m --log-opt max-file=3 --restart=always baiyuetribe/sspanel:ssr
        ...
        install_tool
        check_docker
        docker run -d --name=ssrmu -e NODE_ID=$node_id -e MU_SUFFIX=microsoft.com -e MU_REGEX=%5m%id.%suffix -e API_INTERFACE=glzjinmod -e MYSQL_HOST=$web_ip -e MYSQL_USER=$db_user -e MYSQL_DB=$db_name -e MYSQL_PASS=$user_pwd --network=host --log-opt max-size=50m --log-opt max-file=3 --restart=always baiyuetribe/sspanel:ssr
        greenbg "恭喜您，后端节点已搭建成功"
        end=$(date "+%s")
        echo 安装总耗时:$[$end-$start]"秒"           
	fi       
...
backend_docking_netflix(){
	...
	if [[ "${vnum}" == "1" ]]; then
        ...
        install_tool
        check_docker
        docker run -d --name=ssrmu -e NODE_ID=$node_id -e MU_SUFFIX=microsoft.com -e MU_REGEX=%5m%id.%suffix -e API_INTERFACE=modwebapi -e WEBAPI_URL=$web_url -e WEBAPI_TOKEN=$webapi_token -e DNS_1="$dnsip" -e DNS_2="" --network=host --log-opt max-size=50m --log-opt max-file=3 --restart=always baiyuetribe/sspanel:ssr
        greenbg "恭喜您，后端节点已搭建成功"
        end=$(date "+%s")
        echo 安装总耗时:$[$end-$start]"秒"           
	elif [[ "${vnum}" == "2" ]]; then
